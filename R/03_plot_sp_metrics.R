...
sp.clean$sp.guild <- factor(sp.clean$sp.guild, levels = c("plants","pollinators",
                                                          "herbivores"))
# -------------------------------------------------------------------------
# instead of boxplot, replicate fd plot? nah, make them all boxplots
exc.means <- sp.clean %>% 
  # group_by(guilds, type) %>%
  group_by(intraguild.type, sp.guild) %>%
  summarise(exc.avg = mean(log(prob_excl_mean),na.rm = TRUE),
            exc.sd = sd(log(prob_excl_mean),na.rm = TRUE),
            exc.se = sd(log(prob_excl_mean),na.rm = TRUE)/sqrt(n()))


# plot

# exc.plot <- ggplot(sp.clean, aes(x = sp.guild, y = prob_excl_mean)) + 
exc.plot <- ggplot(sp.clean, aes(x = sp.guild, y = log(prob_excl_mean))) +
  ...
  geom_boxplot(aes(fill = intraguild.type),
               outlier.shape = NA,
               alpha = .4) +
  # geom_pointrange(data = exc.means,aes(x = intraguild.type, y = exc.avg,
  #                                     ymin = exc.avg - exc.sd,
  #                                     ymax = exc.avg + exc.sd,
  ...
  #                                     group = intraguild.type),
                  ...
                  # shape = 21)+
  # geom_jitter(aes(color = intraguild.type)) +
  # facet_grid(.~sp.guild,drop = T) +
  scale_color_OkabeIto(darken = .2,name = "Intraguild \ninteractions") +
  scale_fill_OkabeIto(darken = .2,name = "Intraguild \ninteractions") +
  labs(x = "", y = "log(exclusion probability)") +
  theme_bw() +
