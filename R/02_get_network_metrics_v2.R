      ...
      for(i.plot in plots){
        ...
        for(i.guild in 1:length(guild.combinations)){
          ...
          if(nrow(my.matrix) >= min.node.modularity){
            ...
            linkrank_modularity <- infomap_modularity(A = my.matrix,
                                                      allowed.interactions.mat = matrix(1,nrow = nrow(my.matrix),
                                                                                        ncol = ncol(my.matrix)),
                                                      damping = lr.damping,
                                                      pr.algo = lr.alg)
          ...
          community.metrics$value[pos[which(community.metrics$metric[pos] == "quant_modularity")]] <- linkrank_modularity
...
if(include.null){
  ...
  community.null.metrics <- foreach(i.id = 1:length(id.char),
                                    # .combine=comb.fun, 
                                    .packages = c("tidyverse","infomapecology",
                                                  "ineq","robustbase","igraph",
                                                  "MultitrophicFun")) %dopar% {
                                                    
                                                    source("/home/david/Work/Projects/EBD/multitrophic_feasibility/R/aux_linkRankModularity.R")
                                                    # source("/home/david/Work/Projects/EBD/multitrophic_feasibility/R/aux_allowed_interactions.R")
                                                    ...
                                                    # cat(intraguild.types[i.type],"- rep",i.rep,"-",years[i.year],"- plot",i.plot,"-",
                                                    #     guild.combinations[i.guild],"\n")
                                                    ...
                                                    # -------------------------------------------------------------------------
                                                    # randomize
                                                    # for(i.model in 1:length(null.models)){
                                                    #   for(i.rep in 1:replicates){
                                                    if(my.null == "diagonal dominance"){
                                                      # this reshuffling breaks diagonal dominance 
                                                      # simply by redistributing all interaction strenghts (inter and intra)
                                                      # (redistributing row-wise)
                                                      # but it maintains topology (so no need to watch out for block structure)
                                                      my.matrix.null <- my.matrix
                                                      my.matrix.null[my.matrix.null != 0] <- sample(my.matrix.null[my.matrix.null != 0])
                                                      
                                                    }else if(my.null == "topology"){
                                                    ...
                                                    my.ratio <- mean(diag(my.matrix.null))/mean(my.matrix.null[col(my.matrix.null) != row(my.matrix.null)])
                                                    ...
                                                    # my.kurtosis <- moments::kurtosis(as.numeric(my.matrix))
                                                    my.skew <- robustbase::mc(as.numeric(my.matrix.null))
